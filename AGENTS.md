...
**obsidian wikilink images inside writing posts.** the writing vault is an obsidian vault at `static/writing/` (its `.obsidian/` config and `static/writing/attachments/` are gitignored). pasting an image in obsidian drops it in `static/writing/attachments/` and writes an `![[file.png|500]]` embed. the writing renderer (`src/routes/writing/[slug]/+page.svelte`) converts that obsidian embed syntax to `<img src="${ASSETS}/writing/attachments/<file>" width="...">` at load time — so the image is served from R2, same as all other media. the `|500` (or `|500x300`) part is an optional width/height hint; a non-numeric part becomes alt text. workflow: paste in obsidian → `npm run sync-media` (rclone-copies `static/writing/attachments/` → `r2:personal-website-media/writing/attachments/`) → commit the `.md` + push. (the attachments dir is NOT committed; the image lives on R2.) also note: the writing renderer's `wikilink` marked extension resolves `[[...]]` four ways: `[[#heading]]` → same-page anchor; `[[essay]]` / `[[essay#heading]]` / `[[essay|alias]]` → link to `/writing/<slug>` if the target is a published essay (checked against `src/lib/writingPieces.js`, the shared source of truth also used by the `/writing` index); `[[file.png|500]]`-style image targets → left as raw literal (real image embeds use `![[...]]`, converted to R2 `<img>` in a preprocess step); anything else (a private/unpublished note) → plain muted text (`.wikilink-missing`), never a broken link. so linking TO an essay from another essay works on-site; linking to a private note degrades gracefully. add a new essay's filename to `writingPieces.js` and both the index and cross-essay links pick it up.
...
- **`--depth N`** bounds link-following to N hops PER ROOT (depth 0 = the note alone, 1 = + its direct links, 2 = + one more hop). The depth is stored per-root in `manifest.depths` so it doesn't affect other roots, and it opts that run out of the 30-note guard (depth is the bound instead). Palette equivalents in the `share-to-web` plugin: "Share current note — 1 level deep" / "— 2 levels deep".
- **private content — three markers, all keep `[[links]]` inside them out of the closure too:**
  - **`~[[Note]]`** (private LINK) — a leading tilde. Stays a normal clickable link inside obsidian (the `~` is literal text), but the target is never published and the link is redacted from the exported HTML into a blur bar (`.share-redacted`, matched by the anchor's `data-href`, tilde stripped). `linkKey()` normalizes the target (drops `#heading`/`^block`/`|alias`) to match `data-href`. Don't wrap it in backticks — that makes it inline code, not a clickable link.
  - **`%%private text%%`** (obsidian comment) — hidden in reading view, so webpage-html-export never renders it onto the web at all. Nothing to strip from the HTML; `stripPrivateForClosure()` just removes it before link-scanning so its links aren't followed. Use for anything you don't need to see rendered.
  - **`> [!private]` callout** — a normal, VISIBLE box in obsidian, but `stripPrivateForClosure()` drops it before link-scanning and `stripPrivateCalloutHtml()` removes the whole `data-callout="private"` div (nested divs handled by depth-counting) from the exported page. Use for a private section you still want to see rendered while editing.
  - failsafe: a note referenced with `~[[...]]` anywhere is dropped from the closure even if it's also reached via a normal link (loud warning), so a private note can't leak through a second reference.
- **the script wipes `static/share/` before every export.** the plugin silently skips re-rendering pages whose source notes haven't changed (a cache independent of the onlyExportModified setting), so settings changes (theme, titles, icons) never propagate to existing pages otherwise. missing pages always re-render. don't "optimize" the wipe away.
...
- **the script appends `#main-horizontal{min-height:0}` to main-styles.css after every export** (marker: `share-fix`). without it, flexbox min-height:auto lets the content expand past the viewport inside the centered `#main` column, so the page top clips unreachably (can't scroll up to the title) and the sidebar/graph layout degrades. plugin CSS bug, present in stock 1.9.2 exports.
- after a share deploy, viewers with a cached `main-styles.css` see the broken layout until a hard refresh (the css URL never changes). if someone reports "can't scroll up," have them hard-refresh before debugging.
- **exports use the stock obsidian theme** (`themeName: 'Default'`, set by the script each run). exporting with the vault's Border theme mangled MathJax sizing (tiny/broken glyphs) and rendered its heading accent bars as floating red marks. don't export with vault themes.
